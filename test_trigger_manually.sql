...
-- 5. 해당 사용자들이 public.users에 있는지 확인 (타입에 따라 다른 JOIN)
DO $$
DECLARE
    user_id_type TEXT;
BEGIN
    ...
    ELSE
        -- VARCHAR 타입인 경우
        RAISE NOTICE 'VARCHAR 타입으로 조회합니다.';
        EXECUTE '
        SELECT 
            au.id::text as auth_id,
            au.email as auth_email,
...
-- 6. 간단한 버전 (타입 확인 후 수동 실행)
-- user_id가 UUID인 경우:
-- SELECT au.id::text, au.email, u.user_id::text, u.email, u.role
-- FROM auth.users au
-- LEFT JOIN public.users u ON au.id = u.user_id
-- ORDER BY au.created_at DESC LIMIT 5;

-- user_id가 VARCHAR인 경우:
-- SELECT au.id::text, au.email, u.user_id, u.email, u.role
-- FROM auth.users au
-- LEFT JOIN public.users u ON au.id::text = u.user_id
-- ORDER BY au.created_at DESC LIMIT 5;

-- 7. 트리거 함수 수동 실행 테스트 (가장 최근 사용자로)
-- 주의: 실제로 INSERT하지 않고 함수만 테스트
DO $$
DECLARE
    test_user_id UUID;
