...
export async function createInitialRoadmap(profileId?: string, clientData?: any, counselorId?: string | null, updateOnly: boolean = false) {
    ...
    if (process.env.OPENAI_API_KEY && profileId) {
        ...
        if (ragContext) {
            ...
            if (ragResult?.plan?.length) {
                ...
                console.log('[Roadmap LLM] Q-Net API 호출 시작 (병렬 처리, 최대 5초)')
                // Q-Net API 병렬 호출 (최대 5초 타임아웃)
                ...
                console.log('[Roadmap LLM] Q-Net API 결과 - 자격증:', qualifications.length, '시험일정:', examSchedule.length, '직무역량:', jobCompetency.length)
                ...
                const mapped = ragPlanToMilestones(ragResult, clientData || {}, qualifications, examSchedule)
                info = mapped.info
    ...
    if (!usedRag) {
        ...
        // Q-Net API 호출 (규칙 기반 fallback에서도 사용)
        console.log('[Roadmap] 규칙 기반 - Q-Net API 호출 시작')
        ...
        console.log('[Roadmap] 규칙 기반 - Q-Net API 결과 - 자격증:', qualifications.length, '시험일정:', examSchedule.length)
    ...
    const experience = clientData?.work_experience || ''
        console.log('[Roadmap] 학력:', educationLevel, '전공:', major, '경력:', experience ? '있음' : '없음')
    ...
    const isDevCareerForActions = /개발|엔지니어|의료AI|소프트웨어/i.test(targetJob)
    const phase1Actions = [
