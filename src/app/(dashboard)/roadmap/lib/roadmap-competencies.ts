...
export function computeCompetenciesFromProfile(
    ...
    /** 직무·프로필·상담을 종합해 첫 번째 역량 제목 판단 (첫 토큰이 아닌 직무 유형 기반 요약) */
    const synthesizedFirstTitle = ((): string => {
        const j = (targetJob || '').toLowerCase()
        if (/의사|의료|의과|진료|전문의/i.test(j)) return '진료·전문의 역량'
    ...
    /** 직무 요구 요약 + 프로필·상담 반영 여부를 종합한 설명 */
    const keyPoints =
        concreteCompetencies.length <= 100
            ? concreteCompetencies
            : summarizeToKeyPoints(concreteCompetencies)
    const hasProfile = !!(major || educationLevel || workYears > 0)
    const firstDesc =
        hasProfile || analysisText.length > 0
            ? `${keyPoints} · 프로필·상담 반영`
            : keyPoints
