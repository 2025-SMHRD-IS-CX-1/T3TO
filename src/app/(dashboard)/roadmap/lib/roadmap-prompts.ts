...
- **우선 목표 직종 필수·우대 자격증**을 추천하고, **자격증 출력은 3~5개**로 할 것. 개수가 모자라면 해당 분야 취업에 도움이 되는 **연관 자격증**을 목록에서 골라 포함해 3~5개가 되게 추천해라.
...
[공통 원칙 - 필수 우선, 3~5개, 부족 시 연관 자격증으로 채우기]
- **우선 목표 직종 필수·우대 자격증**을 추천하고, **자격증 출력은 3~5개**로 할 것. 개수가 모자라면 따놓으면 취업에 도움이 되는 **연관 자격증**을 포함해 3~5개가 되게 추천해라. **연관 자격증을 채울 때도 아래 학력별 등급 제한을 반드시 적용**할 것.
...
- IT/개발: 정보처리기사, SQLD, ADsP, 정보보안기사, 빅데이터분석기사, 컴퓨터활용능력, 리눅스마스터 등
- 의료/헬스케어: 의료기기산업기사, 임상심리사, 사회복지사, 보건의료정보관리사 등
- 데이터/AI: ADsP, SQLD, 빅데이터분석기사, 정보처리기사, ADP 등
...
[핵심 원칙]
- **우선 목표 직종 필수·우대 자격증**을 검색 결과에서 골라 추천하고, **자격증 출력은 3~5개**로 할 것. 개수가 모자라면 취업에 도움이 되는 **연관 자격증**을 검색 결과에서 골라 포함해 3~5개가 되게 추천해라. **연관 자격증을 채울 때도 아래 학력별 등급 제한을 반드시 적용**할 것.
...
- **우선 목표 직종 필수·우대 자격증**을 선별하고, **자격증 출력은 3~5개**. 개수가 모자라면 해당 분야 취업에 도움이 되는 연관 자격증(목록에 있는 것만)을 포함해 3~5개가 되게 추천해라. **연관 자격증을 채울 때도 아래 자격조건(학력·경력)을 반드시 적용**할 것.
...
- **분야 혼동 금지**: 목표 직무·전공과 직접 관련된 자격증만 추천할 것. 예: 전기전자 직무에 ADsP·SQLD 등 IT 자격증 추천 금지, 기계 직무에 전기 자격증 추천 금지.`
}
...
[작성 지침]
우선 **목표 직종 필수·우대 자격증**을 검색 결과에서 골라 추천하고, **3~5개** 출력할 것. 개수가 모자라면 취업에 도움이 되는 **연관 자격증**을 검색 결과에서 골라 포함해 3~5개가 되게 할 것. **연관 자격증 포함 시에도 아래 [학력 필터]를 반드시 적용**할 것. 위 웹 검색 결과에 **실제로 언급된 자격증만** 사용. 검색 결과에 없는 자격증은 추천하지 말 것. JSON만 출력.`
}
